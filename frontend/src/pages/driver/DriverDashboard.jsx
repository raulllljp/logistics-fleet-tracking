...
export default function DriverDashboard() {
  ...
  const active = (shipments.data?.shipments || [])
  ...
  return (
    <div className="page-stack">
      <header>
        <p className="eyebrow">READY FOR THE NEXT STOP</p>
        <h1>Driver dashboard</h1>
        <p className="page-description">Your vehicle, your assignments, and your next action.</p>
      </header>
      ...
      {profile.loading ? (
        ...
        <DriverProfileCard driver={profile.data.driver} />
      )}

      <section>
        <div className="section-heading">
          <h2>Next deliveries</h2>
          <Link className="text-action" to="/driver/shipments">
            All assignments →
          </Link>
        </div>
        {feedback && (
          <p role="status" className="booking-success" style={{ marginBottom: '1rem' }}>
            {feedback}
          </p>
        )}
        {shipments.loading ? (
      ...
      <section>
        <div className="section-heading">
          <h2>Current & planned trips</h2>
          <Link className="text-action" to="/driver/trips">
            All trips →
          </Link>
        </div>
        {trips.loading ? (
          ...
          <ErrorState
            ...
            message={trips.error}
            onRetry={trips.refresh}
          />
        ) : (
          <DriverTripsList
            trips={(trips.data.trips || []).filter(trip =>
              ['active', 'planned'].includes(trip.status)
            )}
          />
        )}
      </section>
