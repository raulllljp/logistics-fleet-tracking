...
export default function BookShipmentPage() {
  ...
  const run = async action => {
    ...
    try {
      if (action === 'estimate') {
        ...
        navigate(id ? '/customer/shipments/' + encodeURIComponent(id) : '/customer/shipments', {
          replace: true,
          state: { booked: true },
        })
      }
    } catch (problem) {
      setError(shipmentError(problem))
      setErrors(bookingErrors(problem))
    } finally {
      busy.current = false
      setPending('')
    }
  ...
  return (
    <div className="page-stack booking-page">
      <header>
        <p className="eyebrow">A NEW DELIVERY</p>
        <h1>Book a shipment</h1>
        <p className="page-description">Tell us where it starts, where it’s going, and what you’re sending.</p>
      </header>
      ...
      >
        <fieldset className="booking-fields" disabled={Boolean(pending)}>
          <legend className="sr-only">Shipment details</legend>

          <section className="customer-panel">
            <h2>01 / Pickup & destination</h2>
          ...
          <section className="customer-panel">
            <h2>02 / Shipment & estimate</h2>
            <div className="booking-inputs numeric-fields">
              <Field
                label="Weight (kg)"
                name="weight"
                type="number"
            ...
            <p className="role-note">Enter the delivery distance. Addresses do not calculate it automatically.</p>

            <div className="estimate-row">
              <Button variant="quiet" onClick={() => run('estimate')}>
                {pending === 'estimate' ? 'Estimating…' : 'Estimate price'}
              </Button>
              <div aria-live="polite">
                <span className="estimate-label">Estimated delivery cost</span>
                <strong className="estimate-price">
                  {estimate === null ? 'Get an estimate' : formatCurrency(estimate)}
                </strong>
              </div>
            </div>
            <p className="role-note">Estimate only. No payment is collected here. Booking calculates the cost again.</p>
          </section>

          <Button type="submit" disabled={Boolean(pending)}>
            {pending === 'book' ? 'Booking shipment…' : 'Book shipment'}
          </Button>
        </fieldset>

        <p className="sr-only" role="status">
          {pending ? 'Please wait. Your request is in progress.' : ''}
        </p>
