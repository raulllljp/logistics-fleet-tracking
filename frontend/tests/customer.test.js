...
test('customer components render empty lists, actual chronological events, failure and read-only proof', async () => {
  ...
  try {
    ...
    const { formatShipmentId, formatShipmentStateMessage } = await vite.ssrLoadModule('/src/utils/formatters.js')
    ...
    assert.match(failed, /timeline-marker-failed/)
    const booked = render(ShipmentTimeline, { history: [history[1]], status: 'BOOKED' })
    assert.match(booked, /Booked/)
    assert.match(booked, /In progress/)
    assert.match(booked, /timeline-marker-current/)
    assert.match(booked, /timeline-marker-future/)
    assert.doesNotMatch(booked, /Location:/)
    ...
    assert.equal(formatShipmentId('674a00000000000000000001'), '#000001')
    assert.equal(formatShipmentId(null), '—')
    assert.equal(formatShipmentStateMessage('IN_TRANSIT'), 'Your shipment is on the way.')
